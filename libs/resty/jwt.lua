...
local function verify_jwe_obj(secret, jwt_obj)
  ...
  ngx.log(ngx.ERR , 'MAC KEY : ' , mac_key)
  ngx.log(ngx.ERR , 'ENC KEY : ' , enc_key)
...
function _M.verify_jwt_obj(self, secret, jwt_obj, ...)
  ...
  if alg == str_const.HS256 or alg == str_const.HS512 then
	...
	-- ngx.log(ngx.ERR ,'_M.sign : ' , _M.sign )
    ngx.log(ngx.ERR ,'secret : ' , secret )
    ngx.log(ngx.ERR ,'jwt_obj : ' ,cjson_encode(jwt_obj)  )
    ngx.log(ngx.ERR ,'ret : ' , ret  )
    ngx.log(ngx.ERR ,'jwt_str : ' , jwt_str  )
    if not success then
      -- syntax check
